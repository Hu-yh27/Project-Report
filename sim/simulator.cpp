...
void keyPressed(unsigned char key, int x, int y) {
    switch(key) {
        case 'a':
            keys[0].store(0, std::memory_order_relaxed);
            restart_triggered = true;
            break;
        case 's':
            keys[1].store(0, std::memory_order_relaxed);
            break;
        case 'd':
            keys[2].store(0, std::memory_order_relaxed);
            break;
        case 'f':
            keys[3].store(0, std::memory_order_relaxed);
            break;
        case 'g':
            keys[4].store(0, std::memory_order_relaxed);
            break;
    }
}
void keyReleased(unsigned char key, int x, int y) {
    switch(key) {
        case 'a':
            keys[0].store(1, std::memory_order_relaxed);
            break;
        case 's':
            keys[1].store(1, std::memory_order_relaxed);
            break;
        case 'd':
            keys[2].store(1, std::memory_order_relaxed);
            break;
        case 'f':
            keys[3].store(1, std::memory_order_relaxed);
            break;
        case 'g':
            keys[4].store(1, std::memory_order_relaxed);
            break;
    }
}
...
void reset() {
    ...
    // 重置LED状态
    for (int i = 0; i < 5; i++) {
        leds_state[i].store(1, std::memory_order_relaxed);
    }
...
int main(int argc, char** argv) {
    cout << "Starting VGA simulation..." << endl;
    ...
    cout << "Starting main simulation loop..." << endl;
